...
1. **Loosen win criteria**: Landing anywhere on the target word wins (not just exact character)
2. **Visual highlight**: Render the target word with a colored background so users see where to go
...
| Level | Line ~56 | After `target: { row: 0, col: 16 },` |
| Level | Line ~69 | After `target: { row: 1, col: 12 },` |
| Level | Line ~106 | After `target: { row: 2, col: 9 },` |
| Level | Line ~122 | After `target: { row: 4, col: 0 },` |
| Level | Line ~134 | After `target: { row: 0, col: 22 },` |
| Level | Line ~148 | After `target: { row: 2, col: 21 },` |
| Level | Line ~162 | After `target: { row: 2, col: 10 },` |

(Levels 0-0, 0-1, 0-4 do NOT get `targetWord` — their targets are on punctuation or past-line-end.)
...
After `const level = levels[gs.getCurrentLevel()];` (line 214), add target word range computation:
...
if (!cls && targetWordRange && rowIdx === targetWordRange.row && colIdx >= targetWordRange.start && colIdx < targetWordRange.end) {
    isTargetWord = true;
}
```
...
Tests assert cursor position via `getCursorPos()`, not by checking the win condition. All existing tests should pass unchanged.
