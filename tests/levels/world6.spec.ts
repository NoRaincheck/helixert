...
import { goToWorld, goToLevel, pressKeys, getEditorContent, getMode, getCursorPos, pressEscape } from '../helpers';
...
test.describe('World 6 — Select Mode', () => {
  ...
  test('6-8 Level 6-2 content correct on load', async ({ page }) => {
    ...
    const content = await getEditorContent(page);
    expect(content).toEqual(['Keep this.', 'Delete this line.', 'Keep this too.']);
  });
