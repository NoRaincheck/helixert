...
export function findWordEnd(row, col) {
  ...
  if (c >= newLine.length) {
    if (r < content.length - 1) {
      ...
      return {
        row: r,
        col: Math.min(c, nextLine.length > 0 ? nextLine.length - 1 : 0),
      };
    }
  ...
  return {
    row: r,
    col: Math.min(c, newLine.length > 0 ? newLine.length - 1 : 0),
  };
}
...
export function pasteAfter(text) {
  ...
  if (lines.length === 1) {
    ...
    content[cursor.row] = line.slice(0, insertCol) + text +
...
export function pasteBefore(text) {
  ...
  if (lines.length === 1) {
    const line = content[cursor.row];
    content[cursor.row] = line.slice(0, cursor.col) + text +
...
export function isComplete(targetLines) {
  ...
  const stripTrailing = (arr) => {
    const result = [...arr];
    while (result.length > 0 && trimEnd(result[result.length - 1]) === "") {
      result.pop();
    }
    return result;
  };
