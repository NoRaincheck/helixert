...
export function getBufferDisplay() {
    ...
    if ((b === 'f' || b === 'F' || b === 't' || b === 'T') && _buffer.length === 1) {
        return b + ' → ?';
    }
...
function executeBuffered(key, e) {
    ...
    if (['f', 'F', 't', 'T'].includes(buf)) {
        if (key.length === 1 && !e.ctrlKey && !e.altKey) {
            ...
            else if (buf === 'F') result = tb.findCharBackward(c.row, c.col, key, true);
            else if (buf === 't') result = tb.findCharForward(c.row, c.col, key, false);
            else if (buf === 'T') result = tb.findCharBackward(c.row, c.col, key, false);
...
function executeOperator(op) {
    ...
    if (op === 'd') {
        const deleted = tb.deleteRange(sel.startRow, sel.startCol, sel.endRow, sel.endCol);
        ...
        const yanked = tb.yankRange(sel.startRow, sel.startCol, sel.endRow, sel.endCol);
...
function replayChange(change) {
    if (change.type === 'delete') {
        ...
        gs.setSelectStart({ row: change.range.startRow, col: change.range.startCol });
