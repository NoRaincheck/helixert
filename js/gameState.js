...
export const setCursor = (pos) => { _cursor = { ...pos }; };
export const setCursorRow = (r) => { _cursor.row = r; };
export const setCursorCol = (c) => { _cursor.col = c; };
export const setMode = (m) => { _mode = m; };
export const setCurrentLevel = (l) => { _currentLevel = l; };
export const setCommandLog = (log) => { _commandLog = [...log]; };
export const setYankedText = (t) => { _yankedText = t; };
export const setReplacePending = (p) => { _replacePending = p; };
export const setCountBuffer = (b) => { _countBuffer = b; };
export const setCountBufferAppend = (k) => { _countBuffer += k; };
export const setSearchMode = (m) => { _searchMode = m; };
export const setSearchQuery = (q) => { _searchQuery = q; };
export const setLastSearchQuery = (q) => { _lastSearchQuery = q; };
export const setLastSearchDirection = (d) => { _lastSearchDirection = d; };
export const setSearchMatches = (m) => { _searchMatches = [...m]; };
export const setCurrentMatchIndex = (i) => { _currentMatchIndex = i; };
export const setUsedSearchInLevel = (v) => { _usedSearchInLevel = v; };
export const setNavCountSinceSearch = (n) => { _navCountSinceSearch = n; };
export const setSelectStart = (s) => { _selectStart = s ? { ...s } : null; };
export const setSelectEnd = (e) => { _selectEnd = e ? { ...e } : null; };
...
export const insertContentLine = (row, newLine) => { _content.splice(row, 0, newLine); };
export const removeContentLine = (row) => {
    if (row >= 0 && row < _content.length) return _content.splice(row, 1)[0];
    return null;
};
export const addContentLine = (line) => { _content.push(line); };

// --- Command log ---
export const appendCommandLog = (key) => { _commandLog.push(key); };
export const clearCommandLog = () => { _commandLog = []; };
...
export const pushRedo = (s) => { _redoStack.push(s); };
...
export const getSelectionRange = () => {
    ...
    const startCol = _selectStart.row <= _selectEnd.row ? _selectStart.col : _selectEnd.col;
    const endCol = _selectStart.row <= _selectEnd.row ? _selectEnd.col : _selectStart.col;
...
export const isEscapeKey = (e) =>
    e.key === 'Escape' || (e.key === '[' && e.ctrlKey && !e.shiftKey && !e.altKey && !e.metaKey);
