...
import { levels, worlds, getLevelCount, getLevelsForWorld } from './levels.js';
import {
...
export function updateUI() {
    ...
    try {
        ...
        worlds.forEach(w => { completedByWorld[w.num] = 0; });
        ...
        const worldLevels = getLevelsForWorld(_currentWorld);
        const levelInWorld = worldLevels.findIndex(l => levels.indexOf(l) === gs.getCurrentLevel());
        updateLevelIndicator(_currentWorld, world.name, levelInWorld, worldLevels.length);

        createWorldTabs(_currentWorld, completedByWorld, switchWorld);
        createLevelButtons(levels, gs.getCurrentLevel(), _completedLevels, _currentWorld);
