...
export function renderEditor(
  ...
  if (hasSel) {
    ...
    selMinCol = selectStart.row <= selectEnd.row
      ? selectStart.col
      : selectEnd.col;
    selMaxCol = selectStart.row <= selectEnd.row
      ? selectEnd.col
      : selectStart.col;
  }
  ...
  content.forEach((line, rowIdx) => {
    html += `<div class="line"><span class="line-number">${
      rowIdx + 1
    ...
    for (let colIdx = 0; colIdx <= line.length; colIdx++) {
      ...
      if (isCursor && (mode === "NORMAL" || mode === "SELECT")) {
        ...
        targetWordRange && rowIdx === targetWordRange.row &&
    ...
    if (
      rowIdx === cursor.row && cursor.col >= line.length && mode !== "INSERT"
    ) {
...
export function updateLevelIndicator(
  ...
  levelIndicator.textContent = `World ${currentWorld}: ${worldName} — Level ${
    levelInWorld + 1
  } / ${totalInWorld}`;
}

// --- World tabs ---
export function createWorldTabs(
  currentWorld,
  completedLevelsByWorld,
  onWorldClick,
) {
...
// --- Level buttons ---
export function createLevelButtons(
  levels,
  currentLevel,
  completedLevels,
  currentWorld,
